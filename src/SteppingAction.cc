...
void SteppingAction::UserSteppingAction(const G4Step* step) {
    ...
    // ── Wavelength filter ────────────────────────────────────────────────────
    // Kill photons outside the widest configured SiPM sensitivity window.
    // Broadcom AFBR-S4N66P024M covers 250-900 nm; the legacy S13360 curve
    // extends to 940 nm.  Per-sensor PDE interpolation still returns zero
    // outside the selected model range.
    // EJ-200/EJ-230 emit inside this interval, so this filter has negligible effect on
