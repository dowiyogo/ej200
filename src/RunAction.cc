...
void RunAction::EndOfRunAction(const G4Run* run) {
    ...
    if (IsMaster()) {
        G4cout
            ...
            << "\n  Bar → Mylar crossings    : " << BoundaryCensus::GetBarToMylar()
            << "\n  Mylar → World (escaped)  : " << BoundaryCensus::GetMylarToWorld()
            << "\n  Mylar → Mylar (TIR/refl) : " << BoundaryCensus::GetMylarReflected()
            << "\n  Mylar → SiPM (entering)  : " << BoundaryCensus::GetMylarToSiPM()
