...
void PrintUsage(const char* prog) {
    std::cout
        ...
        << "  --transit-sigma-ps X       Sigma del SPTR en ps. AFBR-S4N66P014M 6x6 mm^2 NUV-MT:\n"
        << "                               Lee 2025 (OV~15.5V): sigma_intrinsico=" << SPTR_LEE_INTRINSIC_SIGMA_PS << " ps, sigma_detector=" << SPTR_LEE_DETECTOR_SIGMA_PS << " ps (PUBLICADO, otro OV)\n"
        << "                               Banco propio (OV=10V): rango 85-92 ps RMS (DERIVADO, pendiente medida formal)\n"
        << "                               NO usar 200 ps (era SPTR del SiPM de Lv et al.)\n"
        << "  --electronics-sigma-ps X   Jitter gaussiano de electronica en ps. JITTER TOTAL DEL FASTIC+ NO MEDIDO.\n"
        << "                               Banco propio: sigma_elec<=18 ps; excluye 30 ps (predicha 44 ps a N=7.5 vs obs 30-37 ps)\n"
        << "                               Los 30 ps anteriores eran simulacion del front-end, no jitter del ASIC.\n"
        << "                               TDC de 25 ps contribuye >=25/sqrt(12)~=7.1 ps. Usar 0 para deshabilitar.\n\n"
        << "Opciones:\n"
