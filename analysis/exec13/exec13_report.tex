...
\scriptsize\textbf{Por qu\'e:} F1 usa 10\,ps/bin: el n\'ucleo de $t_4$ no se resuelve. Este panel usa 2\,ps/bin con zoom al n\'ucleo (modo\,$\pm\coreWindowPS$\,ps), exponiendo la estructura ``core vs cola.''
...
\textbf{Estimador robusto del n\'ucleo (MAD$\times$1.4826):}
\vspace{2pt}
{\footnotesize
\begin{itemize}
  \item $x=-690$\,mm (ID\,16): \textbf{MAD\,$=\coreRunOneMAD$\,ps},
        cola\,=\,\coreRunOneTailPct\,\%
  \item $x=-400$\,mm (ID\,31): \textbf{MAD\,$=\coreRunThreeMAD$\,ps},
        cola\,=\,\coreRunThreeTailPct\,\%
  \item $x=+0$\,mm (ID\,51): \textbf{MAD\,$=\coreRunFourMAD$\,ps},
        cola\,=\,\coreRunFourTailPct\,\%
\end{itemize}
}
\vspace{2pt}
{\footnotesize\textit{Ajuste gaussiano} (\coreBinPS\,ps/bin, modo\,$\pm\coreWindowPS$\,ps):
$\sigma_{\rm core}=$ \coreRunOneSigCore / \coreRunThreeSigCore / \coreRunFourSigCore\,ps;
$\chi^2/\mathrm{ndf}=$ \coreRunOneChiNdf / \coreRunThreeChiNdf / \coreRunFourChiNdf
$\Rightarrow$ fit pobre: MAD m\'as fiable.}\\[3pt]
{\footnotesize\textit{Nota:} $\mathrm{IQR}_{\rm Run4}=\coreRunFourIQR$\,ps est\'a contaminado
por la cola (\coreRunFourTailPct\,\%); se reporta MAD en su lugar.}
\end{column}
...
{\footnotesize
\begin{itemize}
  \item Replicado: 25\,ps/bin, $[\mu\pm2\sigma]$ $\to$
        \coreRunOneSigEx / \coreRunThreeSigEx / \coreRunFourSigEx\,ps
        (\textbf{coincide} con EXEC\_12).
  \item $\sigma_{\rm stat}^{\rm teor}=\sqrt{4}\,\tau_d/\langle N_{pe}\rangle$:
        \coreRunOneSigStat / \coreRunThreeSigStat / \coreRunFourSigStat\,ps
        (consistente).
  \item \textbf{Discrepancia documentada:} ventana estrecha captura
        s\'olo el n\'ucleo prompt; ventana $\pm2\sigma_{\rm RMS}$ captura
        n\'ucleo~+~cola~interna. Abierto: cu\'al citar seg\'un contexto.
\end{itemize}
}
\vspace{3pt}
{\footnotesize\textit{Perspectiva:} $\sigma_{\rm core}\approx\coreMeanSigCore$\,ps y
$\sigma_{\rm fit}\approx\coreMeanSigEx$\,ps son intr\'insecos de simulaci\'on
(sin ruido electr\'onico). La resoluci\'on real estar\'a dominada por el
SPTR ($\sim\coreSPTRps$\,ps), introducido en EXEC\_02b.}
\end{column}
\end{columns}
\end{frame}
