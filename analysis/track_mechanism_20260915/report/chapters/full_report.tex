...
\ReportTable{optical_properties_index}{Índice de refracción registrado por material y campaña.}{tab:intro-index}
\ReportTable{optical_properties_absorption}{Longitud de absorción registrada por material y campaña.}{tab:intro-absorption}
...
Los tres valores históricos $1310.8$, $941.0$ y $701.3$ pe/end se conservan solo como estimaciones históricas de procedencia no reconstruida, porque \texttt{PROJECT\_STATE.md} declara que el diccionario literal de \texttt{napkin.py} no tiene procedencia de corrida. El valor central es \HybridNpePerEnd{} pe/end en geometría híbrida.

\begin{description}
\item[Presupuesto histórico.] Los tres valores anteriores no tienen procedencia de corrida reconstruible; no se usan como resultado central.
\item[\texttt{SE\_gap}.] \NOTAVAILABLE: el guardarraíl L2 conserva el margen RMS, pero no una incertidumbre paired influence-function validada.
\item[SPTR y jitter.] \NOTAVAILABLE: no hay medición del SPTR del AFBR-S4N66P024M ni una cadena electrónica MUSIC/SAMPIC parametrizada.
\item[Región UV.] \NOTAVAILABLE entre 200 y 370 nm para las mediciones dispersivas; el MPT usa un clamp explícito.
\item[EJ--230 dispersivo.] \NOTAVAILABLE: no existe un análogo BC--420 válido en las fuentes usadas; se mantiene como control constante.
\item[WLS y pérdidas reales.] \NOTAVAILABLE: el MPT no incluye WLS ni una caracterización completa del acoplamiento y degradación del reflector.
\item[Muestreo espacial.] \NOTAVAILABLE entre $d=50$ y $200$ mm; es una brecha justo en la transición del régimen Cherenkov.
\item[Rutas aisladas.] Pendiente de reproducibilidad: seis scripts tienen rutas de salida no aisladas por campaña, según el registro de ways-of-working.
\item[Comparación de test beam.] Indicativa: difieren la longitud de barra, la energía del muón y la cadena MUSIC/SAMPIC.
\end{description}
