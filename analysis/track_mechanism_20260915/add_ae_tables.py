...
for keys,g in c.groupby(['material','pulse_model','cfd_fraction']):
 for extreme,gx in g.groupby('extreme'):
  th=np.average(gx.theta_median_deg,weights=gx.n_events_used); ratio=1/np.cos(np.radians(th)); vgroup=189.74206202531644 if keys[0]=='EJ-230' else (166.198 if keys[0]=='EJ-200' else 171.893); r.append(dict(material=keys[0],pulse_model=keys[1],cfd_fraction=keys[2],extreme=extreme,path_over_daxial_exact='NOT_AVAILABLE',path_over_daxial_from_theta_median=ratio,v_group_MPT_mm_ns=vgroup,v_implied_mm_ns=vgroup/ratio,reference_experimental_theta_deg=35.4))
...
meta=json.loads((base/'w6_cfd_fits.meta.json').read_text()); meta.update({'ae1_theta_method':'weighted mean of per-cell theta quantiles from partials; raw event theta values were not persisted','ae2_geometry_method':'exact path/d_axial unavailable; proxy 1/cos(weighted median theta) reported','ae2_exact_ratio':'NOT_AVAILABLE','music_sampic_model_available':False}); (base/'w6_cfd_fits.meta.json').write_text(json.dumps(meta,indent=2)+'\n')
