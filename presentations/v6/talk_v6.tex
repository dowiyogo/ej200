...
\begin{frame}{F1 — TOP $k$-Scan (Full, All Positions)}
  \begin{columns}[T]
    ...
    \textbf{Global choice $k=3$} performs well at center but suboptimally at edges (spikes at $x=\pm200,\pm500\mm$ in $\sigma(x)$ profile).\\[4pt]
    $\sigma_\text{TOP}$ peaks where the $k$-th photon must come from the second-nearest SiPM
    ($d_\text{near}{\approx}25\mm$, $d_\text{second}{\approx}45\mm$ at $x=\pm200,\pm500\mm$);
    at $x=\pm100,\pm600\mm$ ($d_\text{near}{\approx}5\mm$) and $x=0$ (34.5/34.5~mm symmetric)
    the nearest SiPM(s) supply all $k=3$.
  \end{columns}
\end{frame}
