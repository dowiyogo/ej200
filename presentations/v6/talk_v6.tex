...
\begin{frame}{C1 — Full Material Properties (from Materials.cc)}
  ...
  \end{center}
  \vspace{4pt}
  \footnotesize Note: $\sigma_t$ values from GEN-3 END-only geometry (scan\_end\_vikuiti, $m^*$-optimal per position). GEN-3 with TOP SiPMs gives $\sigma_\text{END}=\sigmaENDval$ for EJ-230 at $x=0$ (see FIX-08 on TOP interception).
\end{frame}
