...
\begin{frame}[shrink=5]{G1 — BLUE Weight Derivation / G2 — Covariance Convention Comparison}
  \begin{columns}[T]
    \column{0.52\linewidth}
    \textbf{G1 — BLUE formula:}
    \[
      w_E = \frac{\sigma_T^2 - C_{ET}}{\sigma_E^2 + \sigma_T^2 - 2C_{ET}}
    \]
    $\sigma^2 = \sigma_\text{IQR}^2$ (diagonal); $C_{ET} = \text{Cov}(T_E,T_T)$ empirical (best\_est).\\[3pt]
    At $x=0$: $\sigma_E=\sigmaENDval$, $\sigma_T=\sigmaTOPval$, $C_{ET}=196.3\,\text{ps}^2$.\\
    $C_{ET}/(\sigma_E\sigma_T)=0.241$ (BLUE-consistent $\rho$; \emph{not} the corrcoef 0.16).
    \[
      w_E = \frac{15.20^2 - 196.3}{53.68^2 + 15.20^2 - 2{\times}196.3} = 0.013
    \]
    This is one defensible convention — see G2.\\[3pt]
    \textbf{Formula vs.\ event:}\\
    $\sigma^\text{formula}=15.18\ps$ (Gaussian approximation);\\
    $\sigma_\text{IQR}^\text{event}=15.21\ps$ (IQR of BLUE series).\\
    These are different scale metrics; their agreement is coincidental for non-Gaussian data.

    \column{0.44\linewidth}
    \textbf{G2 — Three covariance conventions}\\[3pt]
    \begin{tabular}{p{1.5cm}p{0.9cm}p{0.8cm}p{1.3cm}}
      \toprule
      Convention & $C_{ET}$ & $w_E$ & $\sigma_\text{IQR}^\text{ev}$ \\
      \midrule
      best\_est & 196.3 & 0.013 & 15.21 ps \\
      (IQR σ² + & & & \\
      \quad emp.\ cov) & & & \\[2pt]
      robust IQR & 80.5 & 0.051 & 15.12 ps \\
      \quad (polariz.) & & & \\[2pt]
      empirical & 196.3 & 0.086 & 15.03 ps \\
      (np.var + & & & \\
      \quad np.cov) & & & \\
      \bottomrule
    \end{tabular}\\[4pt]
    All three lie within $0.18\ps$ — comparable to the bootstrap SE on $\sigma_\text{IQR}$ (${\approx}0.17\ps$ at $n=5000$). The choice of covariance convention is not resolved by this dataset; no design conclusion depends on it.\\[3pt]
    $\sigma_\text{IQR}(w)$ is flat to within one SE over $w\in[0,0.1]$ (total variation $0.26\ps$).\\[3pt]
    \textbf{Qualitative BLUE check:} $w_E$ increases with $\sigma_T$ as expected — $w_E^{\rm analytic}$: $x=0$ ($0.013$) $<$ $x{=}{+}600$ ($0.041$) $<$ $x{=}{+}200$ ($0.106$). Empirical $w^*$ ordering is consistent but not statistically resolved ($n=5000$).
  \end{columns}
\end{frame}
