...
\begin{frame}{Two Photon Populations Drive the Timing}
  \begin{columns}[T]
    ...
    \begin{itemize}
      ...
      \item Path to END $\approx L/(2\cos\theta_c)$
      \item Fast: arrive $\sim\!3.7\ns$ after emission
...
\begin{frame}{Bounces and Reflector Attenuation}
  \begin{columns}[T]
    ...
    \]
    {\footnotesize Napkin: $R=0.98$ (Vikuiti spec).}\\[2pt]
    At $L/2=700\mm$: survival $= 0.95^{85.6} \approx 1.2\%$ (code $R=0.95$). \\[4pt]
...
\begin{frame}{Material Choice: EJ-230 is the Fastest Scintillator Here}
  \begin{columns}[T]
    ...
    \begin{tabular}{lccc}
      ...
      $\sigma_t$ (G4) [ps]      & 47.6  & 52.1 & \textbf{49.6}$^{\dagger}$ \\
      \bottomrule
    \end{tabular}
    \vspace{4pt}\\
    \footnotesize $^\dagger$END-only geometry, $m^*$-optimal.\\[4pt]
    EJ-200 has more photons and wins at $x=0$ (47.6 vs 49.6\ps).
    EJ-230 wins on the \textbf{bar average} (50.5 vs 54.9\ps, lower figure) —
    faster $\tau_\text{rise}$ + $\tau_\text{decay}$ dominate off-centre.
...
\begin{frame}{Geant4 Validates the Napkin}
  \begin{columns}[T]
    ...
    \]
    Geant4 (END-only, $m=8$): $\sigma_\text{END} = 53.7\,\text{ps}$ — within 20\%.
...
\begin{frame}{END Estimator: m-Average of First Photons}
  \begin{columns}[T]
    ...
    \]
    \textbf{Propagation time END} $\sim\!700\mm / (v_g\cos\theta_c) \approx 3.7\ns$ — photons travel $\sim\!900\mm$ path to reach the far end.
...
\begin{frame}{TOP Estimator: $k$-th Order Statistic of Pooled Hits}
  \begin{columns}[T]
    ...
    \]
    vs.\ $\tau_\text{prop}^\text{END} \approx 3.7\ns$.\\[4pt]
...
\begin{frame}{Why TOP Beats END: Near-Field vs Far-Field}
  \begin{columns}[T]
    ...
    \begin{tabular}{lcc}
      ...
      & END & TOP \\
      \midrule
      Path length & $\sim\!900\mm$ & $\sim\!13\mm$ \\
      $\tau_\text{prop}$ & $\sim\!3.7\ns$ & $\sim\!53\ps$ \\
    ...
    \begin{tikzpicture}[scale=0.58, every node/.style={font=\scriptsize}]
      ...
      \node[colEND] at (4.8,-0.25) {$\sim\!900\mm$};
    \end{tikzpicture}\\[4pt]
...
\begin{frame}{Effective Velocity: $v_\text{eff} = \veffEND$}
  \begin{columns}[T]
    ...
    Position resolution: $\sigma_x = \frac{v_\text{eff}}{2}\sigma_{\Delta t}/\sqrt{2} = \sigmax$.
...
\begin{frame}[shrink=3]{BLUE Combination: TOP Carries 99\% of the Weight}
  \begin{columns}[T]
    \column{0.56\linewidth}
    ...
    \]
    Weights from covariance matrix ($\sigma^2 = \sigma_\text{IQR}^2$, $\sigma_\text{IQR} = (Q_{75}-Q_{25})/1.349$):
    \[
      w_\text{END} = \frac{\sigma_T^2 - \text{Cov}}{\sigma_E^2 + \sigma_T^2 - 2\,\text{Cov}}
    \]
    ...
    \begin{tabular}{lc}
      ...
      Quantity & Value \\
      \midrule
      $\sigma_\text{END}$ & $53.68\ps$ \\
      $\sigma_\text{TOP}$ & $15.20\ps$ \\
      $\rho(\text{END},\text{TOP})$ & $0.16$ \\
      $w_\text{END}$ & $\wEND$ \\
      $w_\text{TOP}$ & $\wTOP$ \\
    ...
    \end{tabular}
    \end{center}
    Cross-check: $\sigma_\text{BLUE}^\text{formula} = 15.18\ps$ vs event-by-event $15.21\ps$. $\checkmark$

    \column{0.40\linewidth}
    \textbf{Why $w_\text{END} = 0.013$?}\\[2pt]
    ...
    \textbf{Shared photon pool:} both estimators see some of the same photons ($\rho=0.16$ non-zero) but this effect is small.\\[5pt]
    \textbf{Canonical result} (IQR-based $\sigma^2$): $w_\text{END}=0.013$.
  \end{columns}
\end{frame}
...
\begin{frame}{Global Performance (Deployable: $k=3$, $m=8$)}
  \begin{columns}[T]
    ...
    $\dagger$$w_E$ computed position-by-position with np.var(); canonical IQR-based value at $x=0$: $w_E=0.013$ (see App.~G2).\\[6pt]
    \alert{Peaks at $x=\pm200,\pm500\mm$}: $k=3$ is locally suboptimal there.
    Mean $\sigma_\text{BLUE}=19.5\ps$ across bar.
...
\begin{frame}{Oracle vs Global: The Deployment Cost}
  \begin{columns}[T]
    ...
    Peaks at $x=\pm200,\pm500\mm$ arise because $k^*=2$ is better there but global uses $k=3$.
...
\begin{frame}{Position Measurement: END $\Delta t$ is the Right Method}
  \begin{columns}[T]
    ...
    \[
      ...
      \sigma_x = \frac{v_\text{eff}}{2}\,\frac{\sigma_{\Delta t}}{\sqrt{2}}
    \]
...
\begin{frame}{Design Decision: EJ-230 + 20 TOP + 16 END}
  \begin{columns}[T]
    ...
    \begin{itemize}\setlength\itemsep{3pt}
      ...
      \item Reflector: \textbf{Vikuiti ESR} ($R=0.95$ constant in code)
    ...
    \begin{tikzpicture}[scale=0.60, every node/.style={font=\scriptsize},
      ...
      \node[box,fill=green!20] (c) at (0,3.8) {BLUE combination\\$w_T=0.987$: $\sigma=15.2\ps$};
...
\begin{frame}{Summary}
  ...
  \begin{columns}[T]
    ...
    \begin{itemize}\setlength\itemsep{2pt}
      ...
      \item BLUE: $\sigma=15.2\ps$, $w_E=0.013$
    \end{itemize}
...
\begin{frame}{A1 — Geant4 Surface Definitions}
  \begin{columns}[T]
    ...
    Code value (Materials.cc): $R = 0.95$\\
    Manufacturer Vikuiti ESR spec: $R \approx 0.98$\\[4pt]
    The code comment says ``R=0.98 Vikuiti ESR'' but the actual constant set is 0.95.
    All simulation results in this analysis use \alert{$R=0.95$}.\\[6pt]
    \textbf{Napkin uses $R=0.98$} for reflector attenuation estimates.
    The $\Lambda_\text{refl}$ values assume R=0.98.
    With R=0.95: $\Lambda_\text{refl}^H = 240\mm$ (shorter — more loss).\\[4pt]
    This discrepancy should be corrected in a future simulation campaign.
  \end{columns}
\end{frame}
...
\begin{frame}{B1 — Historical Simulation Generations}
  ...
  \begin{tabular}{p{1.8cm}p{3.2cm}p{3.2cm}p{3.2cm}}
    \toprule
    & \textbf{GEN-1} & \textbf{GEN-2} & \textbf{GEN-3} \\
    \midrule
    Build & end-only (early) & build\_end\_vikuiti & build\_end\_vikuiti \\
    Air gap & No & No & Yes (0.10 mm) \\
    Reflector & \texttt{dielectric\_metal skin} & skin surface, R=0.95 & border surface, R=0.95 \\
    TIR & Eliminated by skin & Natural (no bar surface) & Natural Fresnel \\
    $N_\text{pe}$ & $\sim\!0.37$ (BUG) & — & 701.3 \\
    $\sigma_\text{END}$ & N/A (insufficient pe) & 50.5 ps & 53.7 ps \\
    TOP & No & No & Yes ($N=4,8,14,20$) \\
    Status & \textcolor{red}{\textbf{INVALID}} & superseded & \textcolor{green!60!black}{\textbf{current}} \\
    \bottomrule
  \end{tabular}
  \end{center}
  \vspace{6pt}
  \footnotesize GEN-2 END-only gives $\sigma_\text{END}=50.5\ps$ (smaller than GEN-3 $53.7\ps$) because TOP SiPMs in GEN-3 intercept $\sim\!3\ps$ worth of early photons that would otherwise have reached the END face.
\end{frame}
...
\begin{frame}{C1 — Full Material Properties (from Materials.cc)}
  ...
  \begin{tabular}{lccc}
    ...
    $\sigma_t$ END-only (G4) & 47.6 ps & 52.1 ps & 49.6 ps \\
  ...
  \end{center}
  \vspace{4pt}
  \footnotesize Note: $\sigma_t$ values above are from END-only geometry (GEN-2/GEN-3 no-TOP). GEN-3 with TOP SiPMs gives $\sigma_\text{END}=53.7\ps$ for EJ-230 because TOP SiPMs intercept early photons.
\end{frame}
...
\begin{frame}{E1 — END $\sigma$ vs Position (Full Table)}
  ...
  \end{tabular}
  \end{center}
  \footnotesize $k=3$, $m=8$, $N_\text{TOP}=20$, EJ-230. $\sigma = \sigma_\text{IQR} = (Q_{75}-Q_{25})/1.349$.
\end{frame}
...
\begin{frame}{G1 — BLUE Weight Derivation and Cross-Check}
  \begin{columns}[T]
    \column{0.54\linewidth}
    \textbf{BLUE formula:}
    \[
    ...
    \]
    where $\sigma^2 = \sigma_\text{IQR}^2$ and $C_{ET} = \text{Cov}(T_0^E, T_0^T)$ from empirical sample.\\[4pt]
    At $x=0$: $\sigma_E = 53.68\ps$, $\sigma_T = 15.20\ps$, $C_{ET}/(\sigma_E\sigma_T) = 0.16$.
    \[
      w_E = \frac{15.20^2 - C}{53.68^2 + 15.20^2 - 2C} = 0.013
    \]
    \textbf{Formula cross-check:}
    \[
      \sigma_\text{BLUE}^\text{formula} = \sqrt{\frac{\sigma_E^2\sigma_T^2 - C^2}{\sigma_E^2 + \sigma_T^2 - 2C}} = 15.18\ps
    \]
    Event-by-event $\sigma_\text{IQR}[\text{BLUE}] = 15.21\ps$. $\checkmark$ (0.03 ps agreement)

    \column{0.42\linewidth}
    \textbf{G2 — Two variants of $w_E$}\\[4pt]
    \begin{tabular}{p{1.3cm}p{0.9cm}p{1.5cm}}
      \toprule
      Script & $w_E$ & $\sigma^2$ used \\
      \midrule
      best\_est & 0.013 & $\sigma_\text{IQR}^2$ \\
      v5 & 0.086 & np.var() \\
      \bottomrule
    \end{tabular}\\[6pt]
    Both are self-consistent. The difference is whether $\sigma^2$ in the BLUE formula uses IQR-based or empirical variance.\\[4pt]
    \alert{Canonical: $w_E=0.013$} (IQR-based). The IQR metric is the reported resolution, so consistency requires IQR-based weights.\\[4pt]
    The v5 value ($w_E=0.086$) is from np.var() which includes heavy tails. It's physically less clean but not wrong within its own convention.
  \end{columns}
\end{frame}
...
% ---- Appendix I: Jitter ----
\begin{frame}{I1 — Jitter and Timing Budget}
  \begin{columns}[T]
    \column{0.52\linewidth}
    ...
    \end{itemize}
    \vspace{4pt}
    Current simulation includes: photon emission + propagation + SiPM PDE.\\
    Does not include: SPTR, electronic jitter, optical crosstalk
    (23\% total at 12\,V OV; spurious simultaneous pulse can advance $k$th-order timestamp).\\[4pt]
    \alert{Quoted $\sigma_\text{TOP}=15.2\ps$ is a lower bound.}
    \textit{Note:} $\sigma_\text{SPTR}/\sqrt{k\cdot N_\text{active}}$ applies to averages,
    not to the $k$th order statistic of the TOP estimator.
    ...
    G4: EJ-200 wins at $x=0$ (47.6 vs 49.6\ps); EJ-230 wins on bar average.
  \end{columns}
\end{frame}
