...
\input{../napkin_first_principles/values/napkin_macros}
...
\begin{frame}{Two Photon Populations Drive the Timing}
  \begin{columns}[T]
    ...
    \begin{itemize}
      ...
      \item Reflected ($R=\Rvikuiti$ per bounce) back into bar
...
\begin{frame}{Bounces and Reflector Attenuation}
  \begin{columns}[T]
    ...
    At $L/2=700\mm$: survival $= \Rvikuiti^{85.6} \approx 1.2\%$.\\[4pt]
...
\begin{frame}{The Geant4 Optical Model (GEN-3)}
  \begin{columns}[T]
    ...
    \begin{itemize}\setlength\itemsep{2pt}
      ...
      \item Air$\to$Mylar: \texttt{CreateBarSkinReflector()} as border surface — \texttt{dielectric\_dielectric}, $R=\Rvikuiti$ constant
...
\begin{frame}{Geant4 Validates the Napkin}
  \begin{columns}[T]
    ...
    \begin{tabular}{lcc}
      ...
      \bottomrule
    \end{tabular}\\[1pt]
    $\Delta N_\text{pe}/N_\text{nap} = +23\%$ (END-only).\\[4pt]
...
\begin{frame}{TOP Estimator: $k$-th Order Statistic of Pooled Hits}
  \begin{columns}[T]
    ...
    TOP SiPMs sit along the bar; a hit on the nearest SiPM travels only $\sim\!10\mm$.
    \[
...
\begin{frame}{Why TOP Beats END: Near-Field vs Far-Field}
  \begin{columns}[T]
    ...
    \begin{tabular}{lcc}
      ...
      & END & TOP \\
      \midrule
      Path length & $\NapLhalfmm$--$\NappathTIRlimmm$ & $\sim\!10\mm$ \\
    ...
    The 3rd-order statistic (out of $\sim\!1300$ pooled photons) suppresses random single-photon fluctuations.\\[6pt]
    ...
    \begin{tikzpicture}[scale=0.58, every node/.style={font=\scriptsize}]
      ...
      \node[colTOP] at (1.3,0.65) {$\sim\!10\mm$};
      % Far-field arrow
...
\begin{frame}{Oracle vs Global: The Deployment Cost}
  \begin{columns}[T]
    ...
    Peaks at $x=\pm200\mm$ ($k^*=2$) and $x=\pm500\mm$ ($k^*=1$): global $k=3$ is suboptimal there.\\[3pt]
    {\footnotesize $\sigma_B(x{=}0)=15.0\ps$ (v5, \texttt{np.var} weights) vs $15.21\ps$ (best-estimate IQR weights); definitional, see App.~G2.}
...
\begin{frame}{$N_\text{TOP}$ Scan: Physical Geometry Simulations}
  \begin{columns}[T]
    ...
    20 TOP channels for last 3.4 ps gain (unrelated to the S17 oracle gap).
  \end{columns}
\end{frame}
...
\begin{frame}{Design Decision: EJ-230 + 20 TOP + 16 END}
  \begin{columns}[T]
    ...
    \begin{tabular}{lc}
      ...
      $N_\text{pe}$/end at $x=0$ & $514.9$ (hybrid, $N_\text{TOP}=20$) \\
      \bottomrule
    \end{tabular}
    \end{center}
    Target: $\sigma_t < 100\ps$. \alert{Achieved: $15.2\ps$ intrinsic (see App.~I2) — 6.6× better than target.}
...
\begin{frame}{Summary}
  ...
  \begin{columns}[T]
    ...
    \begin{itemize}\setlength\itemsep{2pt}
      ...
      \item Vikuiti (R=\Rvikuiti) recovers non-TIR photons (+23\%)
...
\begin{frame}{A1 — Geant4 Surface Definitions}
  \begin{columns}[T]
    ...
    \begin{itemize}\setlength\itemsep{1pt}
      \item Type: \texttt{dielectric\_dielectric}
      \item Model: \texttt{unified}; Finish: \texttt{polished}
      \item $R = \Rvikuiti$ constant (wavelength-independent)
    ...
    \textbf{A2 — R=\Rvikuiti{} vs R=0.98}\\[4pt]
...
\begin{frame}{A3 — Air-Gap Geometry Details}
  \begin{columns}[T]
    ...
    \begin{enumerate}\setlength\itemsep{1pt}
      ...
      \item Vikuiti reflectivity at air–Mylar interface ($R=\Rvikuiti$)
    \end{enumerate}
    \vspace{4pt}
...
\begin{frame}{B1 — Historical Simulation Generations}
  ...
  \begin{tabular}{p{1.8cm}p{3.2cm}p{3.2cm}p{3.2cm}}
    ...
    Reflector & \texttt{dielectric\_metal skin} & border surface, R=\Rvikuiti & border surface, R=\Rvikuiti \\
    TIR & Eliminated by skin & Natural Fresnel & Natural Fresnel \\
    $N_\text{pe}$/end & $\sim\!0.37$ (BUG) & 701.3 & 514.9 \\
  ...
  \end{center}
  \vspace{6pt}
  \footnotesize GEN-3 END-only (scan\_end\_vikuiti, $m^*=7$, $x=0$): $\sigma_\text{END}=\sigmaENDonlyval$; GEN-3 with $N_\text{TOP}=20$: $\sigma_\text{END}=\sigmaENDval$ ($\sim\!3\ps$ more). \textbf{Measured TOP yield:} $23.75\%=9\,510\,815/40\,039\,535$ (all-optical TOP hits / generated scintillation photons), EJ-204, EndTop, 70 TOP, $x=0$, $N=2000$; EXEC\_28/B0, \texttt{cell.meta.json}. This does not identify early photons or explain the timing difference.
% Sidecar: /home/rrios/ej200_exec26_20260909/build_nightly_20260910/cells/B0/cell.meta.json
\end{frame}
...
\begin{frame}{B2 — The 1-PE Anomaly (GEN-1 Historical Bug)}
  \begin{columns}[T]
    ...
    \end{itemize}
    \textbf{Fix (exec21-optfix):} Changed to \texttt{dielectric\_dielectric} border surface with $R=\Rvikuiti$, preserving natural TIR at bar–air interface.
...
\begin{frame}{D1 — Detailed Photon Budget (EJ-230)}
  \begin{columns}[T]
    ...
    {\small\begin{tabular}{lrl}
      ...
      \midrule
      \textbf{G4/end (END-only)} & \textbf{701.3} & +refl.-rec. \\
      G4/end (hybrid, $N_\text{TOP}{=}20$) & 514.9 & TOP intercept \\
      TOP interception & $-26.6\%$ & vs END-only \\
      Surplus (END-only) & $+23\%$ & Pop.\ II \\
      \bottomrule
    \end{tabular}}
...
\begin{frame}{G2 — Covariance Convention Comparison}
  ...
  \textbf{Qualitative BLUE check:} $w^*$ increases with $\sigma_T$ as expected ($w_E$ monotone increasing in $\sigma_T^2$ in BLUE). Empirical $w^*$ (biased argmin, 81-point scan on the \emph{same} $n{=}5000$ sample, $\lesssim\!1.25\sigma$ improvement, \textbf{not resolved} — reported as a diagnostic, not a result): $x{=}{+}600$ ($\sigma_T{=}\sigmaTOPxpsix\ps$, $w^*{=}\wstarxpsix$), $x{=}0$ ($\sigma_T{=}\sigmaTOPbval\ps$, $w^*{=}\wstarxzero$), $x{=}{+}200$ ($\sigma_T{=}\sigmaTOPxptwo\ps$, $w^*{=}\wstarxptwo$). Mostly monotone in $\sigma_T$; $x{=}0$ vs $x{=}{+}600$ reversed — statistical noise.
\end{frame}
...
\begin{frame}{J1 — Simulation Provenance and Data Files}
  \begin{columns}[T]
    ...
    \begin{tabular}{ll}
      ...
      Scintillator & EJ-230 \\
      Reflector & Vikuiti ESR ($R=\Rvikuiti$) \\
...
\begin{frame}{K1 --- Photon Balance: Every Terminal Photon Accounted For}
  \small EJ-204, EndTop, 70 TOP, $x=0$, $N=2000$; corrected guard and reflector (EXEC\_28/B0).
  \begin{center}\scriptsize
  \begin{tabular}{lrr}\toprule
    Disjoint terminal group (scintillation only) & Count & Fraction [\%] \\\midrule
Bulk absorption: bar & 9\,099\,837 & 22.7271\\
Bulk absorption: wrap & 197\,707 & 0.4938\\
Other Transportation / none & 5\,114\,686 & 12.7741\\
Explicit world guard & 8\,553\,596 & 21.3629\\
Transportation / BarLV / none & 17\,073\,709 & 42.6421\\
Total scintillation terminals & 40\,039\,535 & 100.0000\\
Ledger residual & 0 & 0.0000\\
    \bottomrule\end{tabular}\end{center}
  \scriptsize Zero residual verifies accounting, not a causal interpretation of every status.
  \texttt{Transportation} is the final step process; it must not be read as a boundary outcome.
  END/TOP hits are observations, not additional disjoint terminal bins.\\[3pt]
  Source: EXEC\_28/B0, \texttt{terminal\_fates\_run0\_scintillation}; grouped sidecar:
  \texttt{build\_exec29\_docs/backup\_photon\_balance} (CSV/ROOT/meta).
  Seeds: \texttt{26092601 8349041}; exact invocation in B0 \texttt{cell.meta.json}.
\end{frame}
% Source: /home/rrios/ej200_exec26_20260909/build_nightly_20260910/cells/B0/terminal_fates_run0_scintillation.meta.json
% Derived table: /home/rrios/ej200_deck_20260910/build_exec29_docs/backup_photon_balance.meta.json


\begin{frame}{K2 --- The Reflector Recycles; TIR Guides}
  \small Ideal specular reflection preserves the angle to the surface normal.
  For parallel faces, a ray outside the TIR acceptance remains outside it after a specular return; the reflector does not randomize its direction into a guided ray.
  \begin{center}\small
  \begin{tabular}{crr}\toprule
    $|x|$ [mm] & END gain at $+x$ & END gain at $-x$ \\\midrule
    200 & 1.0198 & 1.0235 \\
    500 & 1.1082 & 1.0765 \\
    650 & 1.2760 & 1.2819 \\\bottomrule
  \end{tabular}\end{center}
  \small Measured B/A: corrected metal reflector / original dielectric factory,
  with the guard corrected in both. Recovery varies with source position and the associated optical paths; $|x|$ is not a measured photon path length.\\[3pt]
  \scriptsize EJ-204, EndTop, 70 TOP, $N=1000$ per arm and position;
  seeds \texttt{26092601 8349041}. EXEC\_28, \texttt{ab\_comparison} (CSV/ROOT/meta),
  per-cell commands and uncertainties in its provenance.
\end{frame}
% Source: /home/rrios/ej200_exec26_20260909/build_nightly_20260910/ab_comparison.meta.json


\begin{frame}{K3 --- Geant4 11.4: REFLECTIVITY Is Not Always Reflectance}
  \small \texttt{dielectric\_dielectric} + \texttt{polished} (not backpainted):
  the supplied \texttt{REFLECTIVITY} sets survival before the Fresnel decision.
  This statement is tested by a boundary census, not inferred from the label ESR.
  \begin{center}\small
  \begin{tabular}{lrr}\toprule
    Forward air$\to$wrap boundary outcome & Count & Fraction [\%] \\\midrule
    Fresnel reflection & 484\,562 & 13.371145 \\
    Fresnel refraction & 3\,066\,577 & 84.620018 \\
    Boundary absorption & 72\,799 & 2.008837 \\\bottomrule
  \end{tabular}\end{center}
  \small A coefficient of $0.98$ did not produce $98\%$ reflection.
  The measured absorption is close to $2\%$; repeated encounters are correlated,
  so a naive binomial significance is not a calibrated uncertainty.\\[3pt]
  \scriptsize EXEC\_26 Phase 1b: main physics, EJ-204, EndTop, 70 TOP, $x=0$, $N=500$;
  seeds \texttt{26092601 8349041}. Source: \texttt{tables/directional\_states}
  (CSV/ROOT/meta), including exact command and provenance.
\end{frame}
% Source: /home/rrios/ej200_exec26_20260909/build_exec26_phase1b_20260909/run500_ready/tables/directional_states.meta.json
